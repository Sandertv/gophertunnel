...
type Protocol interface {
	...
	// packet ID.
	Packets() packet.Pool
	// ConvertToLatest converts a packet.Packet obtained from the other end of a Conn to a packet.Packet from the most
	// recent protocol. Any packet.Packet implementation in the packet.Pool obtained through a call to Packets that is
	// not identical to the most recent version of that packet.Packet must be converted to the most recent version of
	...
	ConvertToLatest(pk packet.Packet) packet.Packet
	// ConvertFromLatest converts a packet.Packet of the most recent Protocol to a packet.Packet of this specific
	// Protocol. ConvertFromLatest must be synonymous to ConvertToLatest, in that it should convert any packet.Packet to
	// the correct one from the packet.Pool returned through a call to Packets if its payload or ID was changed in this
	// Protocol compared to the latest one.
	ConvertFromLatest(pk packet.Packet) packet.Packet
}
...
func (p proto) ConvertToLatest(pk packet.Packet) packet.Packet   { return pk }
func (p proto) ConvertFromLatest(pk packet.Packet) packet.Packet { return pk }
