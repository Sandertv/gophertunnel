...
type ClientData struct {
	...
	// Nonce is a token generated by the host for each joining player. Once the player joins the
	// world's Xbox Live multiplayer session, the host generates a nonce for the player's XUID and
	// publishes it in [World.Nonces]. The client includes the nonce in ClientData when establishing
...
func (data ClientData) Validate() error {
	...
	} else if _, err := net.ResolveUDPAddr("udp", data.ServerAddress); strings.Contains(data.ServerAddress, ":") && err != nil {
